...
import com.lovecraftpixel.lovecraftpixeldungeon.windows.WndGameInProgress;
...
public class StartScene extends PixelScene {
	...
	public void create() {
		...
		int slotGap = LPDSettings.landscape() ? 5 : 10;
		...
		if (LPDSettings.landscape()) yPos += 8;
	...
	private static class SaveSlotButton extends Button {
		...
		protected void onClick() {
			if (newGame) {
				LovecraftPixelDungeon.scene().add( new WndStartGame(slot));
			} else {
				LovecraftPixelDungeon.scene().add( new WndGameInProgress(slot));
			}
		}
	}
}
