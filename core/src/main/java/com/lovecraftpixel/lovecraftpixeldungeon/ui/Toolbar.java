...
public class Toolbar extends Component {
	...
	protected void layout() {
		...
		int slots = LPDSettings.quickSlots();
		...
		for(int i = 0; i <= 3; i++) {
			...
			if (slots == 4 && width < 152){
				if (width < 138){
					if ((LPDSettings.flipToolbar() && i == 3) ||
							(!LPDSettings.flipToolbar() && i == 0)) {
					...
					if (i == 0 && !LPDSettings.flipToolbar() ||
						i == 3 && LPDSettings.flipToolbar()){
						btnQuick[i].border(0, 2);
						btnQuick[i].frame(106, 0, 19, 24);
					} else if (i == 0 && LPDSettings.flipToolbar() ||
							i == 3 && !LPDSettings.flipToolbar()){
		...
		float right = width;
		switch(Mode.valueOf(LPDSettings.toolbarMode())){
			case SPLIT:
				btnWait.setPos(x, y);
