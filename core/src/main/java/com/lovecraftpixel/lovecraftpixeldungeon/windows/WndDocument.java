...
public class WndDocument extends Window {
	...
	public WndDocument( Document doc ){
		...
		int w = LPDSettings.landscape() ? WIDTH_L : WIDTH_P;
		int h = LPDSettings.landscape() ? HEIGHT_L : HEIGHT_P;
	...
	private static class docPage extends ListItem {
		...
		public boolean onClick( float x, float y ) {
			if (inside( x, y ) && found) {
				LovecraftPixelDungeon.scene().addToFront( new WndStory( doc.pageBody(page) ));
				return true;
			} else {
				return false;
			}
