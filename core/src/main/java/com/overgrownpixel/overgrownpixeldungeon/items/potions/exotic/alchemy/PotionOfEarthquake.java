...
public class PotionOfEarthquake extends ExoticPotion {
    ...
    public void shatter(int cell) {
        ...
        for(int p : PathFinder.NEIGHBOURS9){
            for(int p2 : PathFinder.NEIGHBOURS9){
                Actor actor = Actor.findChar(cell+p+p2);
                if(actor instanceof Mob){
                    ((Mob) actor).damage(((Mob) actor).damageRoll(), this);
                } else {
                    ((Char) actor).damage(((Char) actor).damageRoll(), this);
                }
                if (Dungeon.level.heroFOV[cell+p+p2]) {
                    CellEmitter.get( cell+p+p2 ).start( Speck.factory( Speck.ROCK ), 0.07f, 10 );
