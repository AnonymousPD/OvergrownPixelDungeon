		...
		for (int i = 0; i < langs.size(); i++){
			...
			RedButton btn = new RedButton(Messages.titleCase(langs.get(i).nativeName())){
				...
				protected void onClick() {
					...
					OvergrownPixelDungeon.switchNoFade(TitleScene.class, new Game.SceneChangeCallback() {
						@Override
						public void beforeCreate() {
							LPDSettings.language(langs.get(langIndex));
			...
			if (LPDSettings.landscape() && i % 2 == 1){
				btn.setPos(BTN_WIDTH+1, y-(BTN_HEIGHT + 1));
			} else {
				btn.setPos(0, y);
				y += BTN_HEIGHT;
				if (LPDSettings.landscape()) y++;
			}

			add(btn);
		}
		y = Math.max(MIN_HEIGHT, y);
		resize(LPDSettings.landscape() ? WIDTH_L : WIDTH_P, y);
		...
		if (currLang == Languages.ENGLISH){
			...
			RedButton creditsBtn = new RedButton(Messages.titleCase(Messages.get(this, "credits"))){
				...
				protected void onClick() {
					...
					boolean wide = false;
					if (LPDSettings.landscape() && (reviewers.length + translators.length) > 10){
						wide = true;
					}
